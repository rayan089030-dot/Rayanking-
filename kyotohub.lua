...
getKeyBtn.BackgroundColor3 =Color3.fromRGB(88, 101, 242) -- لون ديسكورد
...
-- ==================== نافذة الأداة الأساسية (مخفية حتى يتم إدخال المفتاح) ====================
...
button.Visible = false -- مخفية لحين التحقق

-- زر Get Key ينسخ رابط الديسكورد
...
-- معالجة مشكلة النسخ واللصق (تنظيف النص وإزالة الفراغات الزائدة لمنع أخطاء النسخ)
submitBtn.MouseButton1Click:Connect(function()
    local userKey = textBox.Text or ""
    -- إزالة الفراغات الزائدة من البداية والنهاية التي تنتج أحياناً عن النسخ واللصق
    ...
    if userKey == CORRECT_KEY then
        ...
        task.wait(1)
        keyFrame:Destroy() -- إخفاء وحذف نافذة المفتاح
        button.Visible = true -- إظهار أداة السكريبت الأساسية
    else
        statusLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
